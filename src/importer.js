...
function normalizeRow(raw, format) {
  ...
  return {
    ...
    publishers: splitMulti(raw.publishers), genre_setting_top: splitMulti(raw.genre_setting_top), genre_setting: splitMulti(raw.genre_setting),
    play_focus: splitMulti(raw.play_focus), campaign_type: splitMulti(raw.campaign_type),
    tone_theme_top: splitMulti(raw.tone_theme_top), tone_theme: splitMulti(raw.tone_theme),
    products: [],
  };
}
...
function rowIssues(payload, rowNumber) {
  ...
  for (const list of [payload.publishers, payload.genre_setting_top, payload.genre_setting, payload.play_focus, payload.campaign_type, payload.tone_theme_top, payload.tone_theme]) {
    const seen = new Set();
    for (const v of list || []) {
...
export async function stagingPreview({ filename, format, content }, actor) {
  ...
  return withTransaction(async (client) => {
    const runR = await client.query(
      `INSERT INTO katalog.import_runs (source_name, source_format, rows_total) VALUES ($1,$2,$3) RETURNING id`,
    ...
    for (let i = 0; i < rawRows.length; i++) {
      const rowNumber = i + 1;
      await client.query(
        'INSERT INTO katalog.staging_games_raw (import_run_id, row_number, raw) VALUES ($1,$2,$3)',
      ...
      for (const iss of issues) {
        allIssues.push(iss);
        await client.query(
          `INSERT INTO katalog.import_issues (import_run_id, row_number, severity, code, field, value, message)
    ...
    await client.query('UPDATE katalog.import_runs SET rows_ok=$1, rows_failed=$2, finished_at=now() WHERE id=$3', [rowsOk, rowsFailed, runId]);
...
export async function getImportRun(runId) {
  const run = await pool.query('SELECT * FROM katalog.import_runs WHERE id=$1', [runId]);
  if (!run.rowCount) throw httpError(404, 'Importlauf nicht gefunden.');
  const issues = await pool.query('SELECT * FROM katalog.import_issues WHERE import_run_id=$1 ORDER BY row_number', [runId]);
  const staged = await pool.query('SELECT * FROM katalog.staging_games_raw WHERE import_run_id=$1 ORDER BY row_number', [runId]);
...
export async function commitImport(runId, actor) {
  ...
  const errorRows = new Set((await pool.query(
    `SELECT DISTINCT row_number FROM katalog.import_issues WHERE import_run_id=$1 AND severity='error'`, [runId],
  ...
  await pool.query('UPDATE katalog.import_runs SET committed=true WHERE id=$1', [runId]);
