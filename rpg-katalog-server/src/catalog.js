...
async function uniqueSlug(client, baseSlug, excludeId = null) {
  ...
  for (;;) {
    const r = await client.query(
      excludeId
        ? 'SELECT 1 FROM games WHERE slug = $1 AND id <> $2'
        : 'SELECT 1 FROM games WHERE slug = $1',
...
async function resolveOrCreate(client, table, name) {
  ...
  const r = await client.query(`SELECT id FROM ${table} WHERE lower(name) = lower($1)`, [trimmed]);
...
async function resolveOrCreateTag(client, kind, name) {
  const trimmed = String(name).trim();
  const r = await client.query('SELECT id FROM tags WHERE kind = $1 AND lower(name) = lower($2)', [kind, trimmed]);
  if (r.rowCount) return r.rows[0].id;
  const slug = slugify(trimmed);
  const ins = await client.query(
    `INSERT INTO tags (kind, name, slug) VALUES ($1, $2, $3)
...
export async function findDuplicates({ title, language, edition, excludeId }) {
  ...
  let sql = `
    ...
    FROM games g JOIN languages l ON l.id = g.language_id
    WHERE l.code = $2
      AND (
...
export async function logAudit(client, { actor = 'admin', action, entity, entityId, diff }) {
  await client.query(
    `INSERT INTO audit_log (actor, action, entity, entity_id, diff) VALUES ($1,$2,$3,$4,$5)`,
...
export async function getAuditLog(limit = 120) {
  const r = await pool.query(
    `SELECT a.*, g.title FROM audit_log a
     LEFT JOIN games g ON a.entity = 'games' AND a.entity_id = g.id::text
...
export async function getGameBySlug(slug, isAdmin) {
  ...
  const [publishers, products] = await Promise.all([
    pool.query(
      `SELECT p.name, gp.is_primary FROM game_publishers gp JOIN publishers p ON p.id = gp.publisher_id
    ...
    pool.query(
      `SELECT pr.title, pt.name AS product_type, pr.binding, pr.edition, pr.language_id, l.code AS language, pr.isbn
       FROM products pr JOIN product_types pt ON pt.id = pr.product_type_id
       LEFT JOIN languages l ON l.id = pr.language_id
...
export async function getMasterData() {
  const [languages, publishers, systemFamilies, productTypes, tags] = await Promise.all([
    pool.query('SELECT code, label_de FROM languages ORDER BY sort_order, label_de'),
    pool.query(`SELECT p.id, p.name, count(gp.game_id)::int AS usage_count
      FROM publishers p LEFT JOIN game_publishers gp ON gp.publisher_id = p.id
      GROUP BY p.id ORDER BY p.name`),
    pool.query(`SELECT sf.id, sf.name, count(g.id)::int AS usage_count
      FROM system_families sf LEFT JOIN games g ON g.system_family_id = sf.id
      GROUP BY sf.id ORDER BY sf.name`),
    pool.query(`SELECT pt.id, pt.name, count(pr.id)::int AS usage_count
      FROM product_types pt LEFT JOIN products pr ON pr.product_type_id = pt.id
...
export async function setFluff(id, fluff, actor) {
  ...
  const r = await pool.query('UPDATE games SET fluff = $1 WHERE id = $2 RETURNING id, title', [v, id]);
...
async function upsertRelations(client, gameId, payload) {
  await client.query('DELETE FROM game_publishers WHERE game_id = $1', [gameId]);
  ...
  for (let i = 0; i < publisherNames.length; i++) {
    ...
    await client.query(
      'INSERT INTO game_publishers (game_id, publisher_id, is_primary) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING',
  ...
  for (const [kind, names] of tagGroups) {
    for (const name of names || []) {
      ...
      await client.query('INSERT INTO game_tags (game_id, tag_id) VALUES ($1,$2) ON CONFLICT DO NOTHING', [gameId, tagId]);
    }
  }

  await client.query('DELETE FROM products WHERE game_id = $1', [gameId]);
  let sortOrder = 10;
  for (const prod of payload.products || []) {
    if (!prod.title || !prod.title.trim()) continue;
    const typeR = await client.query('SELECT id FROM product_types WHERE lower(name) = lower($1)', [prod.product_type]);
    ...
    await client.query(
      `INSERT INTO products (game_id, product_type_id, title, binding, edition, language_id, isbn, sort_order)
...
export async function updateGame(id, payload, actor) {
  ...
  return withTransaction(async (client) => {
    const before = await client.query('SELECT title, status FROM games WHERE id = $1', [id]);
...
export async function setGameStatus(id, status, actor) {
  ...
  return withTransaction(async (client) => {
    const before = await client.query('SELECT status, crunch, narrative FROM games WHERE id = $1', [id]);
    if (!before.rowCount) throw httpError(404, 'Eintrag nicht gefunden.');
    ...
    await client.query(
      `UPDATE games SET status=$1, published_at = CASE WHEN $1='published' AND published_at IS NULL THEN now() ELSE published_at END
...
export async function deleteGame(id, force, actor) {
  ...
  return withTransaction(async (client) => {
    const r = await client.query('SELECT title FROM games WHERE id = $1', [id]);
    if (!r.rowCount) throw httpError(404, 'Eintrag nicht gefunden.');
...
export async function createMasterEntry(kind, name, tagKind, actor) {
  ...
  const r = await pool.query(`INSERT INTO ${cfg.table} ${cols} VALUES (${params}) RETURNING id`, vals);
...
export async function renameMasterEntry(kind, id, name, actor) {
  ...
  const trimmed = String(name).trim();
  if (trimmed.length < 2) throw httpError(400, 'Name muss mindestens 2 Zeichen haben.');
  const r = await pool.query(`UPDATE ${cfg.table} SET name=$1, slug=$2 WHERE id=$3 RETURNING id`, [trimmed, slugify(trimmed), id]);
...
export async function deleteMasterEntry(kind, id, actor) {
  const cfg = masterConfig(kind);
  try {
    const r = await pool.query(`DELETE FROM ${cfg.table} WHERE id=$1 RETURNING id`, [id]);
    if (!r.rowCount) throw httpError(404, 'Nicht gefunden.');
